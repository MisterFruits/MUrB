	  ...
	  modelBufferRef   ((GLuint) 0)
{
	...
	for (int j = 0; j <= (this->nPointsPerCircle / 2); j++) {
		...
		for (int i = 0; i <= this->nPointsPerCircle; i++) {
			...
			this->vertexModel[(i+j*(this->nPointsPerCircle +1))*3 + 0] = cos(verticalAngle) * sin(horizontalAngle);
			this->vertexModel[(i+j*(this->nPointsPerCircle +1))*3 + 1] = sin(verticalAngle);
			this->vertexModel[(i+j*(this->nPointsPerCircle +1))*3 + 2] = cos(verticalAngle) * cos(horizontalAngle);
		}
	}
	...
	if(this->window)
	{
		...
		shadersType[0] = GL_VERTEX_SHADER;   shadersFiles[0] = "src/ogl/shaders/vertex130.glsl";
		shadersType[1] = GL_FRAGMENT_SHADER; shadersFiles[1] = "src/ogl/shaders/fragment130.glsl";
...
template <typename T>
void OGLSpheresVisuInst<T>::refreshDisplay()
{
	if(this->window)
	{
		...
		// but faster.
		glDrawArraysInstanced(GL_LINE_STRIP, 0, this->vertexModelSize * sizeof(GLfloat), this->nSpheres);
